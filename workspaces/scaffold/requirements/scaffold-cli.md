...
**Standalone:** No
**Reason:** This change affects scripts, update behavior, docs, and scaffold maintenance flow.
**Expected Output:** Single CLI entry point with clear update metadata.
**Next Phase:** tech-spec

## Goal

Provide one primary scaffold command that is easy to remember and safe to use in downstream projects.

## Functional Requirements

| ID | Requirement |
|----|-------------|
| SCAFFOLD-REQ-010 | The scaffold shall expose `create`, `lint`, `ingest`, and `update` through `scripts/scaffold.sh`. |
| SCAFFOLD-REQ-011 | The scaffold shall keep the update-managed file list in a simple manifest with one path pattern per non-comment line. |
| SCAFFOLD-REQ-012 | The scaffold shall report deprecated scaffold paths during update without deleting them automatically. |
...
|----------------|-----------|--------|
| SCAFFOLD-REQ-010 | `./scripts/scaffold.sh lint`, `ingest`, `update`, and `create` run from the single script. | Pass |
| SCAFFOLD-REQ-011 | `scaffold.manifest` has no include/exclude directives. | Pass |
| SCAFFOLD-REQ-012 | Update reports paths listed in `scaffold.deprecated` when they exist downstream. | Pass |
...
| SCAFFOLD-REQ-011 | ../raw-input/scaffold-cli.md | Simple manifest semantics |
| SCAFFOLD-REQ-012 | ../raw-input/scaffold-cli.md | Non-destructive stale file handling |
